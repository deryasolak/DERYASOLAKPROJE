...
const AddItems = () => {
  ...
  const addItem = () => {
    if (!itemName || !loanDays) {
      message.error('Lütfen Eşya Ekleyiniz');
      return;
    }
  ...
  const handlePayment = () => {
    form.validateFields()
      .then((values) => {
        message.success('Ödemeniz başarıyla yapıldı. Faturanız kayıtlı mail adresine gönderilecektir.');
        form.resetFields();
        setIsModalVisible(false);
        setSelectedItems([]);
      })
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>Günün Tarihi: {todayDate}</div>
      ...
      <div style={{ display: 'flex', marginTop: '20px' }}>
        <div style={{ marginRight: '20px', flex: 1 }}>
          <Card title="Tüm Eşyalar">
            {items.length === 0 ? <Empty description="Hiçbir Öğe Mevcut Değil" /> : (
              <List
                bordered
                dataSource={items}
                renderItem={(item) => (
        ...
        <div style={{ flex: 1 }}>
          <Card title="Eşyalarım">
            {myItems.length === 0 ? <Empty description="Hiçbir Öğe Mevcut Değil" /> : (
              <List
                bordered
                dataSource={myItems}
                renderItem={(item) => (
      ...
      <div style={{ marginTop: '20px' }}>
        <Card title="Sepete Eklenecekler">
          {selectedItems.length === 0 ? <Empty description="Sepette Ürün Yok" /> : (
            <>
              <List
                bordered
